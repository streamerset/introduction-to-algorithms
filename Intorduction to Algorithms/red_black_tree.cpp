...
namespace ita
{
	...
	/// - 红黑树是真正的在实际中得到大量应用的复杂数据结构：C++STL中的关联容器map,set都是红黑树的应用（所以标准库容器的效率太好了，能用标准库容器尽量使用标准库容器）；\nLinux内核中的用户态地址空间管理也使用了红黑树。
	/// - 红黑权势是许多“<b>平衡的</b>”查找树中的一种（首先：<span style="color:#FF0000 ">红黑树是一种近似平衡的二叉树</span>），它能保证在最坏的情况下，基本的动态集合操作的时间为O(lgn)。
	/// - 通过对任何一条从根到叶子的路径上各个结点着色方式的限制，红黑树确保没有一条路径会比其它路径长出两倍，因而是接近平衡的。
	/// - 一要全是黑结点的满二叉树也满足红黑树的定义。满二叉树的效率本身就非常高啊，它是效率最好的二叉树了，所以说它是红黑树的一个特例；普通的红黑树要求并没有满二叉树这么严格。
	/// - 红黑树之所以这么高效，是因为它是<span style="color:#FF0000 ">近似平衡</span>的，又不要求完全的平衡，减少了维护的代价。在计算机科学中有大量的这样的例子，使用近似的东西来提高效率。如二项堆、斐波那契堆等等数不胜数…
	/// @param	TKey	结点中键的类型
